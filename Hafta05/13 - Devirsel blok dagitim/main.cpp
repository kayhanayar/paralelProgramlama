...
void devirsel_blok_paralel_carpim(
	vector<uint64_t>& A,
	vector<uint64_t>& x,
	vector<uint64_t>& sonucVector,
	uint64_t m,
	...
	uint64_t parcaBoyutu = 64 / sizeof(uint64_t)
)
{

	auto iplikFonksiyonu = [&](const uint64_t& id)
	{
		const uint64_t baslangic = id * parcaBoyutu;
		const uint64_t adimUzunlugu = iplikSayisi * parcaBoyutu;
		
		for (uint64_t altSinir = baslangic; altSinir < m; altSinir += adimUzunlugu)
		{
			const uint64_t ustSinir = std::min(altSinir + parcaBoyutu, m);

			for (uint64_t satir = altSinir; satir < ustSinir; satir ++)
			{
				uint64_t toplam = 0;
				for (uint64_t sutun = 0; sutun < n; sutun++)
					toplam += A[satir * n + sutun] * x[sutun];
...
int main()
{
	...
	TIMERSTOP(carpma)
		...
		devirsel_blok_paralel_carpim(A, x, sonucVector, m, n);
